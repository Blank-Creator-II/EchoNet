(function () {
  // Scope to this card's DOM — works because currentScript is set
  // for each <script src="..."> execution
  const script   = document.currentScript;
  const wrapper  = script.closest('aside').querySelector('.canvas-wrapper');
  ...
  if (wrapper.dataset.mode?.toLowerCase() === 'dark') {
      // Set per-card glow color from the accent
      wrapper.style.setProperty(
          '--glow-color',
          `rgba(${bottomColor.r},${bottomColor.g},${bottomColor.b},0.45)`
      );
  } else {
      wrapper.style.setProperty(
          '--glow-color',
          `rgba(${topColor.r},${topColor.g},${topColor.b},0.45)`
      );
  }
